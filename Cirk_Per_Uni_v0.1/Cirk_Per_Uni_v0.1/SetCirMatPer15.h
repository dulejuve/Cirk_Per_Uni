...
int tvFix15[NumMatrx15], tv15[NumMatrx15]; //array for permutation int tvFix[NumMatrx];
...
void CiklicMatrixPermute15()
{
	...
	for (int y = 1; y<binary; y++)
	{
		...
		{
			//CirMatPer[y][q]=CirMat[y][tvFix[q]-1];//CirMatPer[y][q]=CirMat[y][Per[q]-1];
			CirMatPer15[y][q] = CirMat15[y][tvFix15[q] - 1];//CirMatPer[y][q]=CirMat[y][Per[q]-1];
			for (int bin = 0; bin<NumDigits; bin++)
			{
	...
	TT_PTT256(sizeSbox, binary, 1, tvFix15);

	InverzSTT15(); //Inverse STT, use inverse permutation array
	...
	TT_PTT256(sizeSbox, binary, 2, tvFix15);
...
void SetCirMatPer15()
{
	...
	//	cout << "Start...\n";
		...
		{
			get_permutation(r, NumMatrx, tv15); //get_permutation(r, NumPerEle, tv);
