...
int tvFix17[NumMatrx17], tv17[NumMatrx17]; //array for permutation int tvFix[NumMatrx];
...
void CiklicMatrixPermute17()
{
	...
	for (int y = 1; y<binary; y++)
	{
		...
		{
			//CirMatPer[y][q]=CirMat[y][tvFix[q]-1];//CirMatPer[y][q]=CirMat[y][Per[q]-1];
			CirMatPer17[y][q] = CirMat17[y][tvFix17[q] - 1];//CirMatPer[y][q]=CirMat[y][Per[q]-1];
			for (int bin = 0; bin<NumDigits; bin++)
			{
	...
	TT_PTT256(sizeSbox, binary, 1, tvFix17);

	InverzSTT17(); //Inverse STT, use inverse permutation array
	...
	TT_PTT256(sizeSbox, binary, 2, tvFix17);
...
void SetCirMatPer17()
{
	...
	{
		get_permutation(r, NumMatrx, tv17); //get_permutation(r, NumPerEle, tv);
