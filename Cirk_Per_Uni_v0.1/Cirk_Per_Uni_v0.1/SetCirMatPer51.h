...
int tvFix51[NumMatrx51], tv51[NumMatrx51]; //array for permutation int tvFix[NumMatrx];
...
void CiklicMatrixPermute51()
{
	...
	for (int y = 1; y<binary; y++)
	{
		...
		{
			//CirMatPer[y][q]=CirMat[y][tvFix[q]-1];//CirMatPer[y][q]=CirMat[y][Per[q]-1];
			CirMatPer51[y][q] = CirMat51[y][tvFix51[q] - 1];//CirMatPer[y][q]=CirMat[y][Per[q]-1];
			for (int bin = 0; bin<NumDigits; bin++)
			{
	...
	TT_PTT256(sizeSbox, binary, 1, tvFix51);

	InverzSTT51(); //Inverse STT, use inverse permutation array
	...
	TT_PTT256(sizeSbox, binary, 2, tvFix51);
...
void SetCirMatPer51()
{
	...
	{
		get_permutation(r, NumMatrx, tv51); //get_permutation(r, NumPerEle, tv);
